...
public:
  vector<string> letterCombinations(string digits) {
    std::unordered_map<char, std::vector<std::string>> digitToLetterMap;
    ...
    std::vector<std::string> result = std::reduce(
        transformed.begin(), transformed.end(), std::vector<std::string>{},
        [](const std::vector<std::string> &lhs,
           const std::vector<std::string> &rhs) -> std::vector<std::string> {
          if (lhs.empty()) {
            return rhs;
          }
